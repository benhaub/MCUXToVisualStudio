...
###############################################################################
# Author: Ben Haubrich                                                        #
# File: parseMcuXpressoDebugConfig.bash                                      #
...
help() {
  echo "Usage parseMcuXpressoDebugConfig.bash <debugConfigurationFile>"
}

parseMcuXpressoDebugConfig() {
  ...
  #echo "file name for tasks.vs.json is $TEMPORARY_FILE_NAME"
  ...
  echo "      \"text\": \"target remote ${IP_ADDRESS}:${PORT_NUMBER}\"," >> $TEMPORARY_FILE_NAME
  #Insert it into tasks.vs.json
  LINE_NUMBER=`egrep -n -m1 "target remote" launch.vs.json | cut -f1 -d ":"`
...
####For testing
#parseMcuXpressoDebugConfig evkmimxrt1060_azure_iot_mqtt\ JLink\ Debug.launch
