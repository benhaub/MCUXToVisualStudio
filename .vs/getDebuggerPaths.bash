...
#double path separator (\\) forcing the need to do it twice. The it is done
...
getDebuggerPaths() {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [[ ! -d "${SEGGER_PATH}" ]]; then
      echo "Could not find SEGGER executables. Paths searched were:"
      echo /mnt/c/Program\ Files*
  ...
  doubleUpWindowsPathSeparators $TEMPORARY_FILE_NAME
  #Remove the last double separator cause it should only be one to escape the
  #quotation mark
